...
def register_with_railway():
    """Register this worker's URL with the Railway site."""
    if not RAILWAY_URL:
        log.warning("RAILWAY_URL not set, skipping registration")
        return
    # On Render, the worker URL is provided via env or auto-detected
    worker_url = os.getenv("RENDER_EXTERNAL_URL", "")
    if not worker_url:
        log.warning("RENDER_EXTERNAL_URL not set, skipping registration")
        return
    try:
        r = requests.post(
        ...
        if r.status_code == 200:
            log.info(f"Registered with Railway: {worker_url}")
        else:
...
def main():
    global worker_ready
    log.info("╔══════════════════════════════════════╗")
    log.info("║   9look Worker — Render Mode         ║")
    log.info("╚══════════════════════════════════════╝")

    # Start Flask first so /session endpoint is available immediately
    t = threading.Thread(
    ...
    if os.path.exists(COOKIE_FILE):
        try:
            ...
            log.info(f"Cookies loaded ({len(cookies)} cookies)")
        except Exception as e:
    ...
    try:
        ...
        if "login" in page.url or "auth" in page.url:
            log.warning("Not logged in to IntelScry. POST cookies to /session.")
        else:
            log.info("IntelScry session active — Worker ready!")
            worker_ready = True
    except Exception as e:
        log.error(f"Failed to reach IntelScry: {e}")
